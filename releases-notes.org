...
In the goal to reproducible research, we are starting to rewrite
all scripts about process orig files from statistics to damegender
csv files. Some countries, such as Canada (orig2ca.py) and Italy
(orig2it.py) is using the new way. As soon as possible the only
command will be orig2.py simplifying the processing. Some datasets
has been updated in the process.
