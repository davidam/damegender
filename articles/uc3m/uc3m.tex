...
Linux divides the developers in 1891 males (90.09\%), 78 females
(3.72\%) and 130 unknowns (6.78\%). The number of unknowns is due to
different reasons, but it's so common in Linux that the developer is a
company and not a name of a person.

GNU divides the developers in 215 males (91.49\%), 12 females (5.11\%)
and 7 unknowns (2.98\%) Richard Stallman, the GNU founder returned to
...
(85.53\%), 44 females (4.36\%) and 102 unknowns (10.1\%).
...
It has been explained the application about DameGender application,
...
Searching personal names with strings (ex: Leticia) in search engines
such as Google, reveal that these strings are not being classified
as personal names. This problem can be addressed using data structured
such as JSON-LD, microdata, microformats, rdfa, etc. Another solution
may be to store the name, gender, and frequency data in store in the
servers the open data collection servers and identify the context
of the string is a personal name. This seems like an easy problem
to address for popular sites such as Wikipedia and academic websites.

Search engines that identify strings as personal names may help the
user to determine gender. This is similar to other problems such
as searching streets, products, etc. where additional information
such as maps in streets or prices in products is required.

Other sites such as GitHub or GitLab could provide data about the
gender of the developers in the site or in the software project with
these datasets.

Match sites (Meetic, Tinder, etc.) photos, age, and gender are
important. It may be possible to give to users gender, photo, and
interests from personal names using our open data collection
