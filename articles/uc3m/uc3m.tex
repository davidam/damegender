...
\begin{abstract}
  ...
  taking data from wikipedia, lingüistic studies, scientific sites, ...
...
So, there are an objective in United Nations about gender
...
the industry and another problems not solved in an industrial way such
...
lingüistic~\cite{hutson2016gender,van2020gender,okal2018linguistic},
...
4. An approach based on reproducibility.
...
about non discrimination between males and females. The gender is
about the sex determined in the moment of the birth, although can be
changed in some moment in the life. So, there are discussions about
what is gender referring to these problems. But there are consensus
determining gender, frequency and names with official statistics
released by the institutions in the states.

To measure gender gap has been defined indicators about it. In
...
political participation, poverty, domestical work, gender parity in
...
When the measure about these indicators has been done, it's possible
make decisions improving the situation through conclusions about
researches, for example, in ~\cite{miyake2010reducing} are concluding
that making affirmations about ethical values are being reduced the
gender achievement gap in colleges.
...
gender gap in Internet (access and use) by socioeconomical and gender
...
And in ~\cite{mislove2011understanding} has been analized the Twitter
population including the gender. The gender was infered making queries
...
next bias: coverage , structural, lexical (ex: discriminative words
for women) and visibility.

Computer Science is generating many Forbes billionaries, the public
...
participating contributing code and taking political responsabilities
...
gap infering gender from authors name retrieved from arXiv.
...
In ~\cite{liwicki2011automatic} presents a method infering gender from
...
\subsection{Another ideas related}
...
\subsection{Truehood and Falsehood in names, gender and frequency}
\label{sec:truehood}
...
borns in a year and people using the name in a year. So, we have
...
Damegender contains python scripts to create the different datasets
and publish json files that could be used as a Free API Rest
publishing the json files in sites as github pages, gitlab pages, or
similar sites with free uploads.
...
to the user about the gender. That is similar than another problems
such as streets, products, ... where you are giving additional
information such as maps in streets or prices in products.

Another sites such as github or gitlab could be giving data about
