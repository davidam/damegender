...
\title{An international and free dataset about name, gender and frequency at DameGender}
...
Office. Gender detection tools for personal names is based on API
solutions, providing a free software and open data solution. This will
create a competition in a market without a very strong leader, avoiding
...
in fields with similar works such as Natural Language Processing: gender
detection from the name~\cite{sun2019mitigating}), bibliometrics
studies~\cite{holman2018gender},
journalism~\cite{mislove2011understanding,niemi2017gendered,de2014genero}),
lingüistic studies~\cite{hutson2016gender,al2009socio},
...
There are social inertias to be more comfortable using computers or
programming due to be male or female~\cite{buchbinder2015papel}, with
DameGender we can to study or to research these inertias from empirical
data.

DameGender focused on retrieving data from sources such as
...
indicators (economy, education, politics) were obtained from these
sources.
...
Bibliometrics is an important field to measure science and/or
economy~\cite{esparza2020analisis}. So related to the gender gap in science,
Cassidy R. Sugimoto and colleagues~\cite{lariviere2013bibliometrics}
...
In the beginning, DameGender set the next coding system:
``0 as female'', ``1 as male'', ``2 as unknown''. Currently,
all coding systems standars in the section is being adapted
to the software.
...
DameGender has been merging the official datasets (released by statistical
institutions) generating datasets for several natural languages. In
languages where there not statistical institutions this research has
evaluated to use datasets of names downloaded from Wikipedia
discovering a low quantity of names and not very good quality in the
names downloaded in comparison with the statistical institutions or the
commercial APIs.
...
Argentina\footnote{https://datos.gob.ar/},
...
Switzerland\footnote{https://open.alberta.ca/opendata/frequency-and-ranking-of-baby-names-by-year-and-gender},
...
Iceland\footnote{https://statice.is/},
Norway\footnote{https://www.ssb.no/},
New Zealand\footnote{https://catalogue.data.govt.nz/dataset/most-popular-male-and-female-first-names},
Mexico\footnote{http://datamx.io},
Portugal\footnote{http://centraldedados.pt/nomes_proprios/},
Russia\footnote{https://ngodata.ru/},
Slovenia\footnote{https://www.scb.se},
Sweden\footnote{https://www.scb.se/en/},
the United States of America\footnote{https://www.ssa.gov/oact/babynames/limits.html}
and Uruguay\footnote{https://catalogodatos.gub.uy/dataset/intendencia-montevideo-partidas-de-registro-civil-de-montevideo}
(figure ~\ref{fig:mapamundi}).
...
age of the person. The most common use case is that the name is the
input and the output must be gender, frequency and
...
Genderize is a similar case, with free software, free dataset and
an API where you can retrieve data paying or not depending the
number of names.
...
measure the gender gap inexpensively and to find the fifth
...
Clara Sainz de Baranda and Milagros Sainz Ibañez by the
opportunities making dissemination at UC3M and UOC.
Luz Galvis for the software contributions. Open Knowledge
Foundation by the Open Data Day. Daniel Izquierdo and
Laura Arjona for starting this research field at URJC all those
working with Gema Rodríguez Pérez, Jesús González Barahona
and Gregorio Robles.
